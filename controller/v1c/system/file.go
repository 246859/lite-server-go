...
func (f FileController) Upload(ctx *gin.Context) {
	...
	if len(files) == 0 {
		responseuils.FailWithMsg(ctx, "文件列表为空")
		return
	} else if len(files) > 5 {
		responseuils.FailWithMsg(ctx, "单次上传文件数量不能大于5个")
		return
	}
	for _, file := range files {
		// 单个文件大于 10MB
		if file.Size > 10*1024*1024 {
			responseuils.FailWithMsg(ctx, "单个文件最大不能超过10MB")
			return
		}
	}
	...
	if err != nil {
		...
		responseuils.OkWithMsgAndData(ctx, fileInfo, "文件上传成功")
	}
}
